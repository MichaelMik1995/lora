...
class DialogImage
{
    ...
    openSlider(image_element)
    {
        ...
        var imageViewerHeader = "\
                <div class=\"row background-none pd-2\">"+
                "<div class='column'>"
                "</div>"+
        ...
        var image = "<div class='width-100 content-center'><img style='z-index: 99;' rel=\"easySlider\" id=\"image_es\" class='"+image_size+" mg-auto' src=\""+open_image+"\"></div>"; 
...
class StylizeFunction
{
    ...
    route()
    {
        this.loopSelectedElements("route", (i,a,el) => {
            var get_value = $(el).attr(a);
            var array_explo = get_value.split("[");
            ...
            {
                $(el).on("click", () => {
                    var url_path = get_value.replace(".", "/");
            ...
            else
            {
                //replace . > /
                $(el).on("click", () => {
                    var encode_route = btoa(get_value); //atob
                    
                    var url_string = $.post("/route/get-url", {
                        "route": encode_route,
                        "token": localStorage.getItem("TOKEN"),
                        "method": "default",
                    });
                    
                    url_string.always((data) => {
                       console.log(data);
                    });
...
$(document).ready(function(){
    ...
    {
        var createDiv = "<div title='Kliknutím zavřete obrázek' id=\"eS_imageViewer\" class=\"eS_imageViewer mg-auto-all pd-5 pd-1-xsm pd-1-sm\"></div>";
        ...
        if(resX > resY)
        {
            image_size = "width-auto height-auto";
        }
        else
        {
           image_size = "height-100 width-auto";
        }
