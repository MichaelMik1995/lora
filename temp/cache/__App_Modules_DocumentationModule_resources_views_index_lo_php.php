    ...
    <div class="column-2 column-10-xsm pd-2">
        ...
        <div id="documentation-categories" class="mgy-2 display-0-xsm">
            <?php foreach($categories as $category) : ?>
                <?php if(!empty($category['sheets'])) : ?>
                <details class="content-center-xsm bgr-dark-2 pd-2 mgy-1 bd-round-3" open>
                    <summary class='t-light-2 header-4 header-5-xsm'><?php echo $category['title'] ?></summary>
                    <div>
                        <?php foreach($category['sheets'] as $sheet) : ?>
                        <div onClick="generateSheet('<?php echo $sheet['url'] ?>')" class='pdx-5 cursor-point t-info-hover'><i class="fa fa-file"></i> <?php echo $sheet['title'] ?></div>
                        <?php endforeach; ?>
                    </div>
                </details>
                <?php endif; ?>
