<?php
/**
 * Description of Module Model - UserAnnouncement:
 *
 * This model was created for module: User
 * @author MiroJi
 * Created_at: 1690287095
 */
...
class UserAnnouncement extends User implements ModelDBInterface
{
     ...
     */
    public function getAllUserAnnouncement(string $order_by = "id ASC"): Array
    {
     ...
     */
    public function getUserAnnouncement(string $url): Array
    {
    ...
    {
        $db_query = $this->database->selectNewestRows($this->model_table, "id!=?", [0], 1);
        if(!empty($db_query))
        {
            $content = $db_query["content"];
    ...
    public function insertUserAnnouncement(array $insert_values)
    {
        // Insert new row
        return $this->database->insert($this->model_table, $insert_values);
    }
    
    public function updateUserAnnouncement(array $set, string $url)
    {
        // update row
        return $this->database->update($this->model_table, $set, "url=?", [$url]);
    }
    
    public function deleteUserAnnouncement(string $url)
    {
        // delete row
